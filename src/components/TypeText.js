...
export class Typetext extends Component {
  ...
  componentDidMount(){
    ...
    // this.props.toggleAttribute( this.selectName(this.props.attributes.items[0].displayValue ))
    // console.log('Typetext state', this.state)
    // console.log('obj keys',Object.keys(this.state))
    this.props.toggleAttribute( this.selectName(this.props.attributes.items[0].displayValue ))
  }
  ...
  firstSet=()=>{
    this.setState({ ...this.selectName(this.props.attributes.items[0].displayValue) })
